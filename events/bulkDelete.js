...
client.on("messageDeleteBulk", (messageDeleteBulk) => {
  ...
  const channel = messageDeleteBulk.first().channel
  ...
  log.findOne({ guildID: guild.id }, async (err, data) => { 
    ...
    channel.send({content:`${messageDeleteBulk.size} Messages BulkDeleted in ${channel}. I have attached the BulkDeleted messages in the file below.`,files: [attachment]})
  })
})